// Moodle ThreadExample baseline for Project 2 Section 3.
// Build: cmake --build build --target thread-example
// Run:   ./build/thread-example
//
// Two threads share one mutex + condition_variable. Thread 0 sleeps and
// notifies; thread 1 waits while the other reports busy. This sandbox has no
// Engine/SDL — it only teaches the sync primitives used later in the loop.
#include <thread>
#include <chrono>
#include <mutex>
